[sel, enh,C,theta,theta_noc,kf,kr] = pareto_enhsel(0.01, 1, 0.05);
figure
plot(sel,enh)
...
function [enh, C, theta,theta_noc, kf, kr] = opt_enhance(eps)
    ...
    kf_guess = [1, 1, 2, 1, 1.2];
    kr_guess = [0.1, 0.1, 0.1];

    % Define variables and auxiliary coverages
    % Define bounds to span a range of acceptable rate parameters
   
    kf = optimvar('kf', 5,1, 'LowerBound', 0.01, 'UpperBound', 10);
    kr = optimvar('kr', 3,1, 'LowerBound', 0.01, 'UpperBound', 10);
    C = optimvar('C', 3,1, 'LowerBound', 0.01, 'UpperBound', 1);
    theta = optimvar('theta', 3,1, 'LowerBound', 0.01, 'UpperBound', 1);
    ...
    select_eps = kf(4)*theta(2)*theta(1) - eps*(kf(4)*theta(2)*theta(1)+kf(5)*theta(3)*C(1))==0;
    % Nonlinear d/dt equations
    dBdt = kf(1)*C(2)*(1-sum(theta))-kr(1)*theta(2)-kf(4)*theta(2)*theta(1) == 0;
    dCdt = kf(2)*C(3)*(1-sum(theta))-kr(2)*theta(3)-kf(5)*theta(3)*C(1) == 0;
    dAdt = kf(3)*C(1)*(1-sum(theta))-kr(3)*theta(1)-kf(4)*theta(2)*theta(1) == 0;
    dBdt_noc = kf(1)*C(2)*(1-sum(theta_noc))-kr(1)*theta_noc(2)-kf(4)*theta_noc(2)*theta_noc(1) == 0;
    dAdt_noc = kf(3)*C(1)*(1-sum(theta_noc))-kr(3)*theta_noc(1)-kf(4)*theta_noc(2)*theta_noc(1) == 0;
    ...
    rxn_selvsenh.Constraints.dCdt = dCdt;
...
function [sel, enh,C,theta,theta_noc,kf,kr] = pareto_enhsel(selmin, selmax, delta)
    ...
    enh = zeros(size(sel));
    C = zeros(numel(sel),3);
    theta = zeros(numel(sel),3);
    theta_noc = zeros(numel(sel),2);
    kf = zeros(numel(sel),5);
    kr = zeros(numel(sel),3);
